...
import { useUserOrders, useLogout } from "../hooks/useServiceA";
...
function ActivityCard({
  ...
  return (
    <Link
      to={to}
      className="flex items-center gap-4 p-4 rounded-xl border border-slate-100 hover:border-orange-200 hover:bg-orange-50/30 transition-colors group"
    >
      <div
      ...
      <span className="text-slate-300 group-hover:text-orange-400 transition-colors text-lg">
        →
      </span>
    </Link>
  );
...
export default function UserProfile() {
  ...
  const totalApplications =
    applicationsData?.meta?.total ?? applicationsData?.data?.length ?? 0;
