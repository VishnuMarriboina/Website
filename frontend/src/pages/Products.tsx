...
const IMAGE_MAP: [string, string][] = [
  ["60mm", a1], ["40mm", a2], ["20mm", a3], ["12mm", a4], ["6mm", a5],
  ["m-sand", a6], ["sand", a6], ["dust", a7], ["filler", a7],
  ["wetmix", a8], ["wet mix", a8], ["gsb", a9], ["gravel", a10],
];
...
function Products() {
  ...
  const handleAddToCart = async (productId: string, name: string) => {
    ...
    try {
      ...
      const msg = (err as { message?: string })?.message ?? "Could not add item. Please try again.";
      setModal({ isOpen: true, type: "error", title: "Failed to Add", message: msg });
    } finally {
      setAddingId(null);
    }
  };
      ...
      <div className="bg-slate-900 py-10">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <p className="text-slate-400 text-sm mt-3 max-w-xl mx-auto">
            Simplifying your construction aggregate supply — browse, add to cart, and
            track delivery all from one platform.
          </p>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        {isError && !isLoading && (
          <div className="flex flex-col items-center py-20 gap-4">
            <p className="text-slate-500 text-sm">Failed to load products. Please try again.</p>
        ...
        {!isLoading && !isError && products.length === 0 && (
          <div className="flex flex-col items-center py-20 gap-3">
            ...
            <p className="text-slate-500 text-sm font-medium">
              {debouncedSearch ? `No products matching "${debouncedSearch}"` : "No products available yet."}
            </p>
            {debouncedSearch && (
        ...
        {!isLoading && !isError && products.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {products.map(({ id, name, description, price, stock, category }) => {
                ...
                return (
                  <div key={id}
                    ...
                    <div className="relative h-56 overflow-hidden bg-slate-100">
                      <img src={resolveImage(name)} alt={name}
                    ...
                    <div className="flex flex-col flex-1 p-5">
                      <h2 className="font-semibold text-slate-900 text-sm leading-snug">{name}</h2>
                      <p className="text-xs text-slate-500 mt-2 leading-relaxed flex-1">{description}</p>

                      <div className="mt-3 flex items-center justify-between">
                        {price > 0 ? (
                          <span className="text-base font-bold text-slate-800">
                            ₹{price.toLocaleString("en-IN")}<span className="text-xs font-normal text-slate-400">/unit</span>
                          </span>
                        ) : (
                          <span className="text-xs text-slate-400 italic">Price on request</span>
                        )}
